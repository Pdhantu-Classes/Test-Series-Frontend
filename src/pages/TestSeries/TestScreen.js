...
export default function TestScreen() {
  ...
  useEffect(() => {
    ...
    http
      .get(`http://localhost:5000/getQuestions/${MOCK_PAPER_ID}`)
      .then((res) => {
    ...
    http
      .get("http://localhost:5000/getMockPaperTime", {
        headers: {
    ...
    if (activeQuestionIndex + 1 === questions.length) {
      return (
        <div className="next-button-quiz" onClick={submitQuiz}>
          <div className="next-text-quiz">Submit</div>
        </div>
      );
    } else {
      return (
        <div className="next-button-quiz" onClick={nextClick}>
          <div className="next-text-quiz">Next</div>
        </div>
      );
    }
  };
    ...
    if (activeQuestionIndex > 0) {
      return (
        <div className="prev-button-quiz" onClick={prevClick}>
          <div className="prev-text-quiz">Prev</div>
        </div>
      );
    }
  };
