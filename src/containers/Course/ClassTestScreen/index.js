...
const GET_QUESTIONS_TEST_PRELIMS = API_ENDPOINTS.USERS.GET_QUESTIONS_TEST_PRELIMS;
const POST_RESPONSE_TEST_PRELIMS = API_ENDPOINTS.USERS.POST_RESPONSE_TEST_PRELIMS;
...
export default function TestScreen(props) {
  ...
  const [mockPaperTime, setMockPaperTime] = useState(0);
  ...
  useEffect(() => {
    setLoading(true);
    const PAPER_ID = window.localStorage.getItem('testPaperId')
    setMockPaperId(PAPER_ID)
    setUserId(getUserId());

    http
      .get(GET_QUESTIONS_TEST_PRELIMS.replace("<PAPER_ID>", PAPER_ID))
      .then((res) => {
          setLoading(false)
    ...
    setMockPaperTime(Number(window.localStorage.getItem("paperTime")))
    setTimeLeft(Number(window.localStorage.getItem("paperTime")))
  ...
  function submitResponse() {
    ...
    let body = {
      responses: arr.join(","),
      test_paper_id: mockPaperId,
      user_id: userId,
      paper_time_taken: mockPaperTime - timeLeft,
    };
    http
      .post(POST_RESPONSE_TEST_PRELIMS, body)
      .then((res) => {
        setTimeout(()=>{
          setLoading(false)
      },1000)      
        ...
        setTimeout(()=>{
          history.push('/user/course/classTestSubitResponse')
        },1000)
